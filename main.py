...
def prediction(images, model, tokenizer, device):
    model.eval()
...
class OcrPredictor:
    ...
    def __call__(self, images):
        ...
        pred = prediction(images, self.model, self.tokenizer, self.device)
...
config_json = {
    ...
    "train": {
        "root_path": "train_recognition/images/",
...
predictor = OcrPredictor(
    model_path='C:\\Users\\venv3\\ocr-model-last.ckpt',
    config=config_json
)
...
def read_image_file(file) -> np.ndarray:
    ...
    if image.shape[2] == 4:  # Проверка наличия альфа-канала
